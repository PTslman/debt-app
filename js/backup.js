...
export async function exportBackup() {
    try {
        ...
        if (!encrypted) throw new Error('فشل تشفير البيانات');
        const blob = new Blob([encrypted], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `backup-debts-${new Date().toISOString().split('T')[0]}.encrypted`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        showToast('✅ تم تصدير النسخة الاحتياطية المشفرة', 'success');
        return true;
    } catch (error) {
    ...
    input.onchange = async function(e) {
        ...
        try {
            ...
            try {
                ...
                data = decrypted || JSON.parse(text);
            } catch { data = JSON.parse(text); }
            if (!data.persons || !data.debts) { alert('❌ ملف غير صحيح!'); return; }
            const personsSnapshot = await getDocs(collection(db, "persons"));
            const debtsSnapshot = await getDocs(collection(db, "debts"));
            const batch = writeBatch(db);
            for (const docSnapshot of debtsSnapshot.docs) batch.delete(doc(db, "debts", docSnapshot.id));
            for (const docSnapshot of personsSnapshot.docs) batch.delete(doc(db, "persons", docSnapshot.id));
            await batch.commit();
            for (const person of data.persons) { const { id, ...personData } = person; await addDoc(collection(db, "persons"), personData); }
            for (const debt of data.debts) { const { id, ...debtData } = debt; await addDoc(collection(db, "debts"), debtData); }
